	...
	<div class="brand" flex="dir:top">
		...
		<div class="message" flex="dir:left main:justify cross:center">
			<div class="tag-search" flex="dir:left">
				<span flex-box="0" class='tag'>专题</span>
			</div>
			<div class="add" @click='addspecial'>创建新专题</div>
		</div>
		<prompt></prompt>
		<msgList></msgList>
		<page></page>
	...
	export default {
		...
		methods:{
			changemsg(){
				...
				if(this.now==1){
					BrandApi.getList(this)
				}else{
					this.$store.dispatch('now-change',1)
				}
			},
			addspecial(){
				this.$router.push({path:'/index/cms/special/edit'})
			}
		},
		watch:{
			search:function(val, oldVal){
				this.$store.dispatch('brand-search',val)
			}
		},
		created(){
	    	this.$store.dispatch('now-change',parseInt(sessionStorage.getItem("brand-now-page")) || 1)
		},
		beforeRouteLeave(to, from, next){
			this.$store.dispatch('now-change',Number)
			this.$store.dispatch('brand-search',null)
			next()
		}
	}
</script>
